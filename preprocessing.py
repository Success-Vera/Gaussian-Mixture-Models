...
def remove_stop_words(sentence):
  new_word=[word for word in sentence.split() if word not in stop_words]
...
def convert_to_words(num):
  ...
  if (l == 1):
      number.append(single_digits[ord(num[0]) - 48])
      return
  ...
  while (x < len(num)):
      ...
      if (l >= 3):
          if (ord(num[x]) - 48 != 0):
              number.append(single_digits[ord(num[x]) - 48])#,
                    #end=" ")
              number.append(tens_power[l - 3])#, end=" ")
          ...
          if (ord(num[x]) - 48 == 1):
              sum = (ord(num[x]) - 48 +
                      ord(num[x+1]) - 48)
              number.append(two_digits[sum])
              return

          elif (ord(num[x]) - 48 == 2 and
                ord(num[x + 1]) - 48 == 0):
              number.append("twenty")
              return
              ...
              i = ord(num[x]) - 48
              if(i > 0):
                  number.append(tens_multiple[i])#, end=" ")
              # else:
              #     number.append("", end="")
              x += 1
              if(ord(num[x]) - 48 != 0):
